...
class AccountMove(models.Model):
    ...
    sented_natcom = fields.Boolean(default=False,string='Sent Natcom')
    ...
    amount_in_word_ar = fields.Text()
    amount_in_word_vat_ar = fields.Text()
        ...
        for each in self.env['account.move'].search([]):
            if my_total <= 10:
                ...
                if each.invoice_date_time:
                    ...
                    #                     print('each',each)
                    # each.invoice_nat_time = each.invoice_date +" "+each.datetime_field.time()
                    import datetime
    ...
    def invoice_email_sent(self):
        self.attach_ubl_xml_file_button()
